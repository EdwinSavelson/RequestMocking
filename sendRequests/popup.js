    ...
    function renderRequests(requests) {
      ...
      requests.forEach((request, index) => {
        ...
        requestDiv.innerHTML = `
          ...
          <button id="resend-${index}">Resend on Refresh</button>
        `;
        ...
        document.getElementById(`resend-${index}`).addEventListener('click', () => {
          const updatedBody = document.getElementById(`body-${index}`).value;
          try {
            JSON.parse(updatedBody); // validate JSON
            requests[index].body = updatedBody;
            chrome.storage.local.set({ requests: requests }, function() {
              chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
                chrome.tabs.sendMessage(tabs[0].id, { action: 'resendRequests', requests: [requests[index]] });
              });
            });
          } catch (e) {
            alert('Invalid JSON format!');
